        ...
        if (script.showGeneral){
			...
			if (script.useEnableReflections){
				...
				if (script.gameObject.activeInHierarchy){
                    if (script.suiLayerMasks != null && script.suiLayerMasks.Count > 0)
                    {
                        // Ensure the reflectLayer index is within the valid range
                        script.reflectLayer = Mathf.Clamp(script.reflectLayer, 0, script.suiLayerMasks.Count - 1);

                        // Use the MaskField with a valid array for the layers
                        script.reflectLayer = EditorGUI.MaskField(new Rect(rt.x + margin + 120, rt.y + basePos + 48, 90, 18),
                                                                  "", script.reflectLayer, script.suiLayerMasks.ToArray());
                    }
                    else
                    {
                        // Handle the case where the mask options are empty or invalid
                        EditorGUI.LabelField(new Rect(rt.x + margin + 120, rt.y + basePos + 48, 90, 18), "No layers available");
                    }
                }
    ...
    if (script.editorIndex == 1){
        ...
        if (script.showUnderwater){
           ...
           // GUI.contentColor = colorDisabled;
			...
			script.underwaterFogDist = EditorGUI.Slider(new Rect(rt.x+margin+165, rt.y+200, setWidth, 18),"",script.underwaterFogDist,0.0f,10000.0f);
